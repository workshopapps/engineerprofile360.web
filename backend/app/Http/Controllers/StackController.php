...
class StackController extends Controller
{
    public function addStack(StackRequest $request): JsonResponse
    {
        ...
        try {
            ...
            return $this->sendResponse(
                true,
                'Error occurred while creating Stack',
                $e->getMessage(),
    ...
    public function updateStack(StackRequest $request, $stack_id): JsonResponse
    {
        try {
            ...
            if (empty($stack_id)) {
                return $this->sendResponse(
                    true,
                    "Expected id of an existing stack. None given",
                    "stack id is missing.",
                    Response::HTTP_BAD_REQUEST
                );
            }
            if (empty($request->name)) {
                return $this->sendResponse(
                    true,
                    "Expected a valid stack 'name'. None give",
                    "stack name is missing.",
                    Response::HTTP_BAD_REQUEST
                );
            }
            ...
            if ($user_id !== $uid) {
                return $this->sendResponse(
                    true,
                    "Not authorised to update this stack",
                    "Unauthorised.",
                    Response::HTTP_BAD_REQUEST
                );
            }
            ...
            return $this->sendResponse(
                true,
                "Something went wrong updating stack " . $e->getMessage(),
                'Failed updating stack.',
                Response::HTTP_INTERNAL_SERVER_ERROR
            );
        }
    }
